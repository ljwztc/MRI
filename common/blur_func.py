...
class MotionModel:
    ...
    def __call__(self, kspace, blurriness_type, seed=None, clone=True) -> torch.Tensor:
        ...
        odd_err = (2 * np.pi * scale) * torch.rand(1, generator=g).numpy()
        even_err = (2 * np.pi * scale) * torch.rand(1, generator=g).numpy()

        line_0 = width * torch.rand(1, generator=g)
        if blurriness_type == 'rigid':
            for line in range(width):
                if line % 2 == 0:
                    rand_err = even_err #* np.abs(line - width/2)
                else:
                    rand_err = odd_err #* np.abs(line - width/2)
                phase_error = torch.from_numpy(np.exp(-1j * rand_err))
                phase_matrix[:, :, line] = phase_error
        elif blurriness_type == 'respiration':
            for line in range(width):
                if line % 2 == 0:
                    rand_err = even_err * np.sin(scale * np.abs(line - width/2) * np.pi + scale)
                else:
                    rand_err = odd_err * np.sin(scale * np.abs(line - width/2) * np.pi + scale)
                phase_error = torch.from_numpy(np.exp(-1j * rand_err))
                phase_matrix[:, :, line] = phase_error
